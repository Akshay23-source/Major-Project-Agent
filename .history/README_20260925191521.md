...
For release builds, set `EXPO_PUBLIC_API_URL` to the public HTTPS URL of the deployed backend.
 ...
 BUYER            MARKETPLACE (Node :5000)        AGRI AGENT (Node :4000)         AGENT / DRIVER apps
   │                      │                               │                               │
 1 │── place order ──────▶│ save in `orders`              │                               │
   │◀─ "Order Placed" ────│ (escrow locked if blockchain) │                               │
   │                      │                               │                               │
 2 │                      │── POST /api/integrations/ ───▶│ check API key                 │
   │                      │   marketplace/orders          │ save in `agri_orders`         │
   │                      │◀── 201 trackingId AGRI-… ─────│ status PENDING ──────────────▶│ 3 Agent sees
   │                      │   syncStatus = SYNCED         │ + notification                │   "New Order"
   │                      │                               │                               │
   │                      │                               │◀── Accept ────────────────────│ 4 Agent accepts
   │                      │                               │◀── Dispatch (driver+vehicle) ─│ 5 Agent assigns
   │                      │◀── status update ─────────────│ PICKUP_ASSIGNED               │
   │                      │                               │                               │
   │                      │                               │◀── driver taps each step ─────│ 6 Driver app
   │                      │◀── status update + GPS ───────│ PICKED_UP → IN_TRANSIT →      │
   │◀─ push notification ─│   order: packed / shipped     │ OUT_FOR_DELIVERY → DELIVERED  │
   │                      │                               │                               │
 7 │◀─ "Delivered" ───────│ order = delivered             │ driver + vehicle freed        │
