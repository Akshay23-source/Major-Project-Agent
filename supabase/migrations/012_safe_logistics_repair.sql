-- 012_safe_logistics_repair.sql
-- This script ensures all logistics tables, columns, and policies exist.
-- It is designed to be completely safe to run multiple times.
...
-- We cannot use IF NOT EXISTS for ADD TABLE, so we wrap it in a PL/pgSQL block to ignore errors if it already exists
DO $$
BEGIN
